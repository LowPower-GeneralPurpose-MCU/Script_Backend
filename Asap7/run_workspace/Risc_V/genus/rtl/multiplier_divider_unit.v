...
module divider (
    ...
    reg [31:0] quotient;
    // One extra bit is required during the restoring-division shift.  A
    // 32-bit remainder overflows for divisors with bit 31 set.
    reg [32:0] remainder;
    reg [31:0] mask;
    reg invert_res;
    reg div_inst_q;
            ...
            STATE_BUSY: begin
                for (step = 0; step < BITS_PER_CYCLE; step = step + 1) begin
                    if (mask_tmp != 32'd0) begin
                        ...
                        if ((rem_tmp >= {1'b0, divisor_abs}) &&
                            (divisor_abs != 32'd0)) begin
                            rem_tmp = rem_tmp - {1'b0, divisor_abs};
