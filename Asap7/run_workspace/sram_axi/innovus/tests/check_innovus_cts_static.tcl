...
require_contains $sram_route_guard {createRouteBlk} "SRAM signal-only macro-body route blockage"
require_contains $sram_route_guard {-exceptpgnet} "SRAM route blockage exempts PG special routes"
require_contains $sram_route_guard {set SRAM_ROUTE_GUARD_LAYERS {M6 M7}} "SRAM route guard avoids M4/M5 pin-access layers"
...
require_contains $innovus_master {catch {error $post_place_pg_drc_error}} "master flow hard-errors on post-place PG DRC"
require_contains $innovus_pnr {catch {error $post_place_pg_drc_error}} "PnR flow hard-errors on post-place PG DRC"
