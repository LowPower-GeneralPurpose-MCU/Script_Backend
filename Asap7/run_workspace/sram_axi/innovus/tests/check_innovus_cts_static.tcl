...
foreach flow [list $innovus_master $innovus_pnr] {
    ...
    require_contains $flow {verify_pg_special_drc_or_stop ./verify_rpt/sram_m4_interface_drc.rpt {M4 M4}} "separate SRAM M4 interface DRC guard"
    require_contains $flow {verify_pg_special_drc_or_stop ./verify_rpt/pg_drc_after_trim.rpt {M5 M9}} "post-place actionable PG DRC guard before placed save"
...
require_contains $innovus_master {error $post_place_pg_drc_error} "master flow raises Tcl error on post-place PG DRC"
require_contains $innovus_pnr {error $post_place_pg_drc_error} "PnR flow raises Tcl error on post-place PG DRC"
...
require_contains $flow_checks {-layerChangeRange {M4 M5}} "SRAM blockPin stitch stays on local M4/M5"
require_contains $flow_checks {-targetViaLayerRange {M4 M5}} "SRAM blockPin target vias stay below the core mesh"
